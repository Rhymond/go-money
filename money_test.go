...
func TestMoney_Add(t *testing.T) {
	...
	for _, tc := range tcs {
		m, err := New(tc.amount1, "EUR")
		if err != nil {
			t.Error(err)
		}
		om, err := New(tc.amount2, "EUR")
		if err != nil {
			t.Error(err)
		}
...
func TestMoney_Chain(t *testing.T) {
	...
	r := m.Add(om).Divide(5).Multiply(4).Subtract(om).amount.val

	if r != 7 {
		t.Errorf("Expected %d got %d", 7, r)
	}
}
